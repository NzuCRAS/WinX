...
class Cdn {
  ...
  static List<Cdn> _fromJson(String data) {
    ...
    String getUrl(Map<String, dynamic> map) {
      final url = map['url'];
      if (url == null) {
        throw XdnmbApiException('找不到 url');
      }
...
class Timeline implements ForumBase {
  ...
  /// 时间线的最大页数，大于 [maxPage] 的均返回页数为 [maxPage] 的内容，默认为 20
  @override
  final int maxPage;
...
class HtmlForum implements ForumBase {
  ...
  /// 版块的最大页数，大于 [maxPage] 的均返回页数为 [maxPage] 的内容，版块的最大页数总是 100
  @override
  int get maxPage => 100;
  ...
  /// 从 HTML 数据构造 [HtmlForum]，[forumId] 为版块 ID
...
abstract interface class PostBase {
  ...
  /// 主串的回串数量，包含被删除的串，回串的 [replyCount] 为 0
  int? get replyCount;
...
class ForumThread {
  ...
  /// 主串的最后的回复，最多 5 个。
  ///
  /// 由于删串的原因，即使 [mainPost] 的`replyCount`大于 5，[recentReplies] 的长度也不一定等于 5，
  /// 而且 [mainPost] 的`replyCount`小于等于 5 时和 [recentReplies] 的长度也不一定相等。
...
class Thread {
  ...
  /// 主串某一页的回复
  ///
  /// [replies] 长度为 0 时可能这一页和后面的页数都没有回复，
...
class Feed extends FeedBase {
  ...
  static List<Feed> _fromJson(String data) {
    ...
    int parseIntLoose(dynamic v, {int fallback = 0}) {
    ...
    int readReplyCount(Map<String, dynamic> map) {
      // 服务端字段名存在差异：常见为 reply_count，但也可能是驼峰或其它别名。
      // 这里尽量兼容，确保客户端能拿到“总回复数”。
      if (map.containsKey('reply_count')) {
        return parseIntLoose(map['reply_count']);
      }
      if (map.containsKey('replyCount')) {
        return parseIntLoose(map['replyCount']);
      }
      if (map.containsKey('replies')) {
        return parseIntLoose(map['replies']);
      }
      if (map.containsKey('reply')) {
        return parseIntLoose(map['reply']);
      }

  // 2026-03: 部分 feed 返回会裁剪字段，仅包含基础字段（id/fid/img/ext/now/user_hash/...）。
  // 此时服务端并未提供“总回复数”，客户端无法凭空推导，只能回退为 0。
  // 这里保留扩展点：如果未来服务端改用其它字段名，可以继续在此补充。
      // 最后回退：保持旧行为。
      return parseIntLoose(map['reply_count'] ?? '0');
    }
    ...
    return <Feed>[
      ...
      userId: int.tryParse(map['user_id'] ?? '0') ?? 0,
            ...
            replyCount: readReplyCount(map),
      recentReplies: (map['recent_replies'] ?? '').toString().isEmpty
        ? <int>[]
...
/// 图片类型，目前 X 岛只支持`jpeg`、`png`、`gif`三种图片格式
enum ImageType {
  jpeg,
  png,
  gif;
...
class XdnmbApi {
  ...
  /// 获取随机封面图的最终 URL。
  ///
  /// 服务端会返回 302，并通过 `Location` 指向实际图片路径（通常是 `/h/<hash>.jpg`）。
  ///
  ...
  Future<Uri> _followRedirects(Uri url, {int maxRedirects = 5}) async {
    if (maxRedirects <= 0) {
      throw XdnmbApiException('重定向次数过多');
    }
    ...
    if (streamed.statusCode >= 300 && streamed.statusCode < 400) {
      ...
      if (loc == null || loc.trim().isEmpty) {
        throw XdnmbApiException('重定向接口未返回 Location');
      }

      // Location may be absolute or relative.
      final resolved = url.resolve(loc.trim());
      // 递归跟随重定向
      return _followRedirects(resolved, maxRedirects: maxRedirects - 1);
    } else if (streamed.statusCode == 200) {
      // 已经到达最终 URL
      return url;
    } else {
      throw XdnmbApiException('获取随机封面图失败，状态码：${streamed.statusCode}');
    }
  }

  /// 获取 CDN 列表
  ///
  /// [cookie] 为饼干的 cookie 值
  ...
  /// 获取网页版版块信息
  ///
  /// [forumId] 为版块 ID，[cookie] 为饼干的 cookie 值
  Future<HtmlForum> getHtmlForumInfo(int forumId, {String? cookie}) async {
    if (forumId <= 0) {
      throw XdnmbApiException('版块 ID 要大于 0');
    }
  ...
  /// 获取版块里的串
  ///
  /// [forumId] 为版块 ID，[page] 为页数，最小值为 1，最大值为 100，[cookie] 为饼干的 cookie 值
  ///
  /// 一页最多 20 串
  Future<List<ForumThread>> getForum(int forumId,
    ...
    if (page <= 0) {
      throw XdnmbApiException('页数要大于 0');
    }
  ...
  /// 获取时间线里的串
  ///
  /// [timelineId] 为时间线 ID，[page] 为页数，最小值为 1，最大值根据 [Timeline.maxPage]，
  /// [cookie] 为饼干的 cookie 值
  ///
  /// 一页最多 20 串。
  ...
  /// 获取帖子（串）内容
  ///
  /// [mainPostId] 为主串 ID，[page] 为页数，最小值为 1，[cookie] 为饼干的 cookie 值
  ///
  /// 一页最多 19 个回复
  ///
  /// 没有饼干的话只能浏览前 100 页
  Future<Thread> getThread(int mainPostId,
      {int page = 1, String? cookie}) async {
    if (mainPostId <= 0) {
      throw XdnmbApiException('主串 ID 要大于 0');
    }
    if (page <= 0) {
      throw XdnmbApiException('页数要大于 0');
    }
  ...
  /// 获取串引用
  ///
  /// [postId] 为串 ID，[cookie] 为饼干的 cookie 值
  Future<Reference> getReference(int postId, {String? cookie}) async {
    if (postId <= 0) {
      throw XdnmbApiException('串的 ID 要大于 0');
    }
  ...
  /// 获取订阅
  ///
  /// [feedId] 为订阅 ID，[page] 为页数，最小值为 1，[cookie] 为饼干的 cookie 值
  ///
  /// 一页最多 10 个订阅
  ...
  /// 获取网页版订阅
  ///
  /// [page] 为页数，最小值为 1，[cookie] 为饼干的 cookie 值
  ///
  ...
  /// 添加订阅
  ///
  /// [feedId] 为订阅 ID，[mainPostId] 为主串 ID，[cookie] 为饼干的 cookie 值
  ...
  /// 网页版添加订阅
  ///
  /// [mainPostId] 为主串 ID，[cookie] 为饼干的 cookie 值
  ...
  /// 发表新串
  ///
  /// [forumId] 为版块 ID，[content] 为串的内容，[name] 为串的名称，
  /// [email] 为串的邮箱，其实没用，[title] 为串的标题，
  /// [watermark] 为是否添加图片水印，默认不添加，
  /// [image] 为串的图片，[cookie] 为饼干的 cookie 值
  ///
  ...
  /// [email] 为串的邮箱，其实没用，[title] 为串的标题，
  /// [watermark] 为是否添加图片水印，默认不添加，
  /// [imageFile] 为图片文件路径，[cookie] 为饼干的 cookie 值
  ///
  /// 发串需要饼干（[xdnmbCookie] 或 [cookie]）
  ...
  /// 回串
  ///
  /// [mainPostId] 为要回复的主串 ID，[content] 为串的内容，[name] 为串的名称，
  /// [email] 为串的邮箱，其实没用，[title] 为串的标题，
  /// [watermark] 为是否添加图片水印，默认不添加，
  /// [image] 为串的图片，[cookie] 为饼干的 cookie 值
  ///
  ...
  Future<void> userLogin(
    ...
    if (!hasPhpSessionId) {
      throw XdnmbApiException('用户登陆需要 PHPSESSID');
    }
  ...
  /// 获取饼干信息
  ///
  /// [cookieId] 为饼干 ID，[userCookie] 为用户的 cookie 值
  ///
  ...
  Future<XdnmbCookie> getCookie(int cookieId, {String? userCookie}) async {
    ...
    if (userCookie == null) {
      throw XdnmbApiException('获取饼干需要用户 Cookie');
    }
    final response =
  ...
  Future<CookiesList> getCookiesList({String? userCookie}) async {
    ...
    final idList = parent!.querySelectorAll('tr td:first-child').map((e) {
      ...
      if (next == null) {
        throw XdnmbApiException('获取饼干 ID 失败');
      }
  ...
  /// 获取新饼干
  ///
  /// [verify] 为验证码，[userCookie] 为用户的 cookie 值
  ///
  ...
  Future<void> getNewCookie(
    ...
    if (!hasPhpSessionId || userCookie == null) {
      throw XdnmbApiException('获取新饼干需要 PHPSESSID 和用户 Cookie');
    }
  ...
  /// 删除饼干
  ///
  /// [cookieId] 为饼干 ID，[verify] 为验证码，[userCookie] 为用户的 cookie 值
  ///
  ...
  Future<void> deleteCookie(
    ...
    userCookie = userCookie ?? _userCookie;
    if (!hasPhpSessionId || userCookie == null) {
      throw XdnmbApiException('删除饼干需要 PHPSESSID 和用户 Cookie');
    }
...
(int, int?) _getHtmlPostId(Element parent,
  ...
  if (element == null) {
    throw XdnmbApiException('HTML 里没找到 id');
  }
  final str = _RegExp._parseNum.stringMatch(element.innerHtml);
  if (str == null) {
    throw XdnmbApiException(idNotExist ?? 'HTML 里没找到 id');
  }
...
(String, String) _getHtmlPostImage(Element parent) {
  ...
  if (element == null) {
    ...
    if (img == null) {
      throw XdnmbApiException('HTML 没找到 image');
    }
    final match = _RegExp._parseThumbImage.firstMatch(img);
    if (match == null) {
      throw XdnmbApiException('HTML 里没找到 image');
    }
    image = match[1]!;
...
DateTime _getHtmlPostTime(Element parent) {
  ...
  if (element == null) {
    throw XdnmbApiException('HTML 里没找到 postTime');
  }

  return _parseTimeString(element.innerHtml);
}

/// 从 HTML 里获取串用户的 userhash 和用户是否管理员（红名）
(String, bool) _getHtmlPostUserHash(Element parent) {
  ...
  if (element == null) {
    throw XdnmbApiException('HTML 里没找到 userHash');
  }
...
String _getHtmlPostName(Element parent) {
  ...
  if (element == null) {
    throw XdnmbApiException('HTML 里没找到 name');
  }
...
String _getHtmlPostTitle(Element parent) {
  ...
  if (element == null) {
    throw XdnmbApiException('HTML 里没找到 title');
  }
...
String _getHtmlPostConent(Element parent) {
  ...
  if (element == null) {
    throw XdnmbApiException('HTML 里没找到 content');
  }
